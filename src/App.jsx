...
function HomePage({ ctx, goto }) {
  const { me, official, isChair, isSecretary, activeMembers, savings, loans, announcements, mySavingsTotal, groupSavingsTotal, totalLoansOut, totalFinesCollected } = ctx;
...
function LoansPage({ ctx }) {
  ...
  const decide = async (loan, decision) => {
    ...
    if (!isLarge(loan)) {
      await persist.updateLoan(loan.id, { status: "approved", dateApproved: todayISO(), approvals: ["chair"] });
      await persist.log("loan_approved", `${me.name} approved ${fmtKES(loan.amount)} loan for ${memberName(loan.memberId)}`);
        ...
        {showForm && (
          <Card style={{ marginTop: 14 }}>
            <form onSubmit={submitRequest}>
              ...
              <div style={{ fontSize: 11, color: "#A79B78", marginBottom: 10 }}>
