...
import { signIn, signUp, signOut, getSession, onAuthChange, verifySignupCode, resendSignupCode } from "./lib/auth.js";
...
function LoginScreen({ onLogin, goRegister, notify }) {
  ...
  const submit = async (e) => {
    ...
    try {
      ...
      if (!me) {
        // Auth account exists but the member profile row is missing. With
        // supabase/schema.sql's handle_new_user trigger in place this
        // should no longer happen (the row is created transactionally
        // with the auth account) — this is a defense-in-depth fallback
        // for projects that haven't run that migration yet, or for a
        // row that was deleted directly. Let the user complete their
        // profile here instead of a dead-end.
...
// The OTP switch is dynamic, not comment-driven (see src/lib/auth.js and
// supabase/schema.sql): registration always signs up then immediately tries
// to sign in. If Supabase requires email confirmation, that sign-in fails
// with an "Email not confirmed" error, which is caught below and turns into
// the verification-code screen automatically — no code edits needed either
// way, just the Supabase dashboard setting.
function RegisterScreen({ goLogin, notify }) {
  ...
  // The member row for this user already exists the instant the auth
  // account does (see the `handle_new_user` trigger in
  // supabase/schema.sql) — this just fills in the details the trigger
  // couldn't know. Role and status are deliberately left untouched here:
  // the trigger decided them server-side, atomically, which is also what
  // closes the race condition where two people registering at the same
  // instant could both have computed "0 active members" client-side and
  // both become Chairperson.
  ...
  const submitForm = async (e) => {
    ...
    setBusy(true);
    try {
      // Step 1: create the Supabase Auth account. If it already exists
      // (e.g. the user retried after a dropped connection), signUp
      // returns the existing user without error — that's fine, the
      // profile row from the first attempt is still there to update.
      await signUp(form.email.trim(), form.password.trim());

      // Step 2: sign in immediately. Succeeds right away in development
      // (email confirmation off). In production with confirmation on,
      // this throws — caught below, switching to the OTP step.
      let session;
      try {
        ({ session } = await signIn(form.email.trim(), form.password.trim()));
      } catch (err) {
      ...
      if (!session?.user) throw new Error("Sign-in after registration failed.");

      await finishProfile(session.user.id);
    } catch (err) {
