...
/* ---------------------------------------------------------------
   ...
   Design tokens:
   --nyish-ink      #16302A  deep ledger green (primary / header)
...
function TopBar({ title, subtitle, onMenu, right, menuIcon: MenuIcon = Menu }) {
  return (
    <div
      style={{
        background: "linear-gradient(160deg,#16302A,#1E3F35)",
        color: "#F7F2E4",
...
function Btn({ children, onClick, variant = "primary", full, disabled, type = "button", icon }) {
  const styles = {
    primary: { background: "#16302A", color: "#F7F2E4" },
    gold: { background: "#C99A2E", color: "#16302A" },
    ghost: { background: "transparent", color: "#16302A", border: "1px solid #D8CCA8" },
...
function AuthShell({ children }) {
  ...
  return (
    <div
      style={{
        minHeight: 560,
        background: "radial-gradient(circle at 30% 0%, #23473C, #101F1B 65%)",
        display: "flex",
...
function LoginScreen({ members, onLogin, goRegister, notify }) {
  ...
  const submit = (e) => {
    ...
    setBusy(false);
    if (!m) {
      notify("No match. Check your phone number and PIN.");
      return;
    }
    if (m.status === "pending") {
  ...
  return (
    <Card style={{ background: "#FFFDF8" }}>
      <div style={{ fontFamily: "Fraunces, serif", fontSize: 18, fontWeight: 600, color: "#16302A", marginBottom: 14 }}>
        Member sign in
      </div>
      <form onSubmit={submit}>
        ...
        <Field label="PIN">
          <input style={inputStyle} type="password" inputMode="numeric" value={pin} onChange={(e) => setPin(e.target.value)} placeholder="4-digit PIN" required />
        </Field>
      ...
      <div style={{ marginTop: 14, fontSize: 11, color: "#A79B78", textAlign: "center", lineHeight: 1.5 }}>
        First time setting up? Admin sign-in: {BOOTSTRAP_PHONE} / {BOOTSTRAP_PIN}
      </div>
    </Card>
  );
}
...
function RegisterScreen({ members, onRegistered, goLogin, notify }) {
  const [form, setForm] = useState({ name: "", phone: "", idNumber: "", email: "", pin: "" });
  const [busy, setBusy] = useState(false);
  ...
  const submit = async (e) => {
    ...
    if (form.pin.trim().length < 4) {
      notify("Choose a PIN of at least 4 digits.");
      return;
    }
    setBusy(true);
  ...
  return (
    <Card style={{ background: "#FFFDF8" }}>
      ...
      <form onSubmit={submit}>
        ...
        <Field label="Create a PIN">
          <input style={inputStyle} type="password" inputMode="numeric" value={form.pin} onChange={set("pin")} placeholder="4+ digits" required />
        </Field>
...
function SectionTitle({ children, action }) {
  return (
    <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", margin: "20px 4px 10px" }}>
      <div style={{ fontFamily: "Fraunces, serif", fontSize: 15.5, fontWeight: 600, color: "#16302A" }}>{children}</div>
      {action}
    </div>
  );
}
    ...
    <div>
      ...
      <div style={{ padding: "16px 16px 4px" }}>
        <div style={{ display: "flex", gap: 12 }}>
          <Card style={{ flex: 1 }}>
            <div style={{ fontSize: 11.5, color: "#8B8264", fontWeight: 600 }}>{official ? "Group savings" : "My savings"}</div>
            <div style={{ fontFamily: "Fraunces, serif", fontSize: 21, fontWeight: 700, color: "#16302A", marginTop: 4 }}>
              {fmtKES(official ? groupTotal : mySavingsTotal)}
            </div>
          </Card>
          <Card style={{ flex: 1 }}>
        ...
        {latestAnnouncements.length === 0 ? (
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
            {latestAnnouncements.map((a) => (
              <Card key={a.id}>
                <div style={{ fontWeight: 700, fontSize: 14, color: "#16302A" }}>{a.title}</div>
...
function QuickAction({ icon, label, onClick }) {
  return (
    <button onClick={onClick} style={{ background: "#FFFDF8", border: "1px solid #EAE0C4", borderRadius: 14, padding: "14px 10px", display: "flex", flexDirection: "column", alignItems: "center", gap: 6, cursor: "pointer", color: "#16302A" }}>
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
            {sorted.map((s) => (
              <Card key={s.id} style={{ display: "flex", alignItems: "center", justifyContent: "space-between" }}>
                <div>
                  {official && <div style={{ fontWeight: 700, fontSize: 13.5, color: "#16302A" }}>{memberName(s.memberId)}</div>}
                  <div style={{ fontSize: 11.5, color: "#A79B78" }}>{s.date}{s.note ? ` · ${s.note}` : ""}</div>
                </div>
                <div style={{ fontFamily: "Fraunces, serif", fontWeight: 700, color: "#1B4332" }}>+{fmtKES(s.amount)}</div>
              </Card>
            ))}
          </div>
        )}
            ...
            {sorted.map((l) => (
              <Card key={l.id}>
                <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start" }}>
                  <div>
                    {official && <div style={{ fontWeight: 700, fontSize: 13.5, color: "#16302A" }}>{memberName(l.memberId)}</div>}
                ...
                <div style={{ display: "flex", flexWrap: "wrap", gap: 8 }}>
                  {activeMembers.map((m) => (
                    <button type="button" key={m.id} onClick={() => toggleAttend(m.id)}
                      style={{
                        border: "1px solid " + (attendance.includes(m.id) ? "#1B4332" : "#D8CCA8"),
                        background: attendance.includes(m.id) ? "#DCE9E1" : "#FFFDF8",
                        color: "#16302A", borderRadius: 999, padding: "6px 12px", fontSize: 12.5, cursor: "pointer",
                      }}>
                      {m.name.split(" ")[0]}
                    </button>
                  ))}
                ...
                <Card key={mt.id}>
                  <button onClick={() => setOpenId(open ? null : mt.id)} style={{ background: "none", border: "none", width: "100%", textAlign: "left", cursor: "pointer", padding: 0 }}>
                    <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
                      <div>
                        <div style={{ fontWeight: 700, fontSize: 13.5, color: "#16302A" }}>{mt.date}</div>
            ...
            return (
              <button key={it.key} onClick={() => open(it.key)} style={{ display: "flex", alignItems: "center", justifyContent: "space-between", background: "#FFFDF8", border: "1px solid #EAE0C4", borderRadius: 14, padding: "14px 16px", cursor: "pointer" }}>
                <div style={{ display: "flex", alignItems: "center", gap: 12 }}>
                  ...
                  <span style={{ fontSize: 14, fontWeight: 600, color: "#16302A" }}>{it.label}</span>
                </div>
      ...
      {pending.length > 0 && (
        <>
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
            {pending.map((m) => (
              <Card key={m.id}>
                <div style={{ fontWeight: 700, color: "#16302A" }}>{m.name}</div>
        ...
        {active.map((m) => (
          <Card key={m.id} style={{ display: "flex", alignItems: "center", justifyContent: "space-between" }}>
            <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
              ...
              <div>
                <div style={{ fontWeight: 700, fontSize: 13.5, color: "#16302A" }}>{m.name}</div>
...
function CertificatePage({ ctx, onBack }) {
  ...
  useEffect(() => {
    ...
    ctx2d.strokeStyle = "#16302A";
    ctx2d.lineWidth = 1.5;
    ctx2d.strokeRect(36, 36, W - 72, H - 72);

    ctx2d.textAlign = "center";
    ctx2d.fillStyle = "#16302A";
