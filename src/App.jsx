...
export default function NyishApp() {
  ...
  const persist = {
    ...
    // Activity log — stores entries in nyish_store under key "activityLog".
    // Non-fatal: if it fails for any reason it only swallows the log entry,
    // it does NOT crash the calling action (approval, loan grant, etc.).
    log: async (action, detail) => {
      try {
        ...
        const entry = { id: uid(), actorId: me.id, action, detail, at: new Date().toISOString() };
        const next = [entry, ...existing].slice(0, 500); // cap at 500 entries
        await supabase.from("nyish_store").upsert({ key: "activityLog", value: JSON.stringify(next) });
      } catch (err) {
        // Non-fatal — log failures must never crash an action.
...
function MembersPage({ ctx, onBack }) {
  ...
  const setRole = async (m, role) => {
    ...
    await persist.log("role_changed", `${me.name} appointed ${m.name} as ${roleLabel(role)}`);
