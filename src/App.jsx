...
import {
  ...
  CheckCircle2, XCircle, Landmark, User, RefreshCw
} from "lucide-react";
...
// Loans at or above this amount require both the Chairperson AND the
// Treasurer to approve before funds are released — a standard chama
// safeguard against a single official approving large payouts alone.
const LARGE_LOAN_THRESHOLD = 50000;
// Flat interest applied once at approval time (principal × rate), the
// common chama convention — not compounding, not monthly amortization.
// Change this single number if your group's AGM sets a different rate.
...
// Re-fetches the latest list right before merging, instead of writing a
// possibly-stale in-memory copy. Narrows (doesn't eliminate — see
// PROGRESS.md) the "two people save at once, one overwrites the other"
// race from the blob-storage scheme.
async function appendToList(key, item) {
  const latest = await loadList(key);
  const next = [...latest, item];
  const ok = await saveList(key, next);
  return { ok, next };
}
async function updateInList(key, id, patch) {
  const latest = await loadList(key);
  const next = latest.map((x) => (x.id === id ? { ...x, ...(typeof patch === "function" ? patch(x) : patch) } : x));
  const ok = await saveList(key, next);
  return { ok, next };
}
...
async function logActivity(actorId, action, detail) {
  await appendToList("activityLog", { id: uid(), actorId, action, detail, at: new Date().toISOString() });
}
...
export default function NyishApp() {
  ...
  useEffect(() => {
    (async () => {
      const [m, s, l, mt, an, co, fi, rot, log] = await Promise.all([
        loadList("members"),
        loadList("savings"),
        loadList("loans"),
        loadList("meetings"),
        ...
        loadList("fines"),
        loadText("rotation", null),
        loadList("activityLog"),
      ]);
      setMembers(m);
      setSavings(s);
      setLoans(l);
      ...
      setConstitution(co);
      setFines(fi);
      setRotation(rot ? JSON.parse(rot) : { order: [], currentIndex: 0, cyclesCompleted: 0 });
      setActivityLog(log);
      setLoading(false);
    })();
  }, []);
  ...
  const totalLoansOut = loans.filter((l) => ["approved", "active"].includes(l.status)).reduce((a, b) => a + Number(b.balance), 0);
  const totalFinesCollected = fines.filter((f) => f.status === "paid").reduce((a, b) => a + Number(b.amount), 0);
  const activeMembers = members.filter((m) => m.status === "active");

  const persist = {
    // Bulk replace — use only when a full-array rewrite is genuinely intended
    // (e.g. bulk role edits). Prefer the append/update helpers below for
    // anything that adds or changes a single record, since those re-fetch
    // the latest data first and so are safer against two people saving at
    // the same time. See PROGRESS.md "Foundation" section.
    ...
    fines: async (next) => { setFines(next); await saveList("fines", next); },

    appendSaving: async (item) => { const { next } = await appendToList("savings", item); setSavings(next); },
    appendLoan: async (item) => { const { next } = await appendToList("loans", item); setLoans(next); },
    updateLoan: async (id, patch) => { const { next } = await updateInList("loans", id, patch); setLoans(next); },
    appendMeeting: async (item) => { const { next } = await appendToList("meetings", item); setMeetings(next); },
    appendAnnouncement: async (item) => { const { next } = await appendToList("announcements", item); setAnnouncements(next); },
    updateMember: async (id, patch) => { const { next } = await updateInList("members", id, patch); setMembers(next); },
    removeMember: async (id) => { const { next } = await removeFromList("members", id); setMembers(next); },
    appendFine: async (item) => { const { next } = await appendToList("fines", item); setFines(next); },
    updateFine: async (id, patch) => { const { next } = await updateInList("fines", id, patch); setFines(next); },
    setRotation: async (next) => { setRotation(next); await saveText("rotation", JSON.stringify(next)); },
    reloadSavings: async () => { const next = await loadList("savings"); setSavings(next); },

    log: (action, detail) => logActivity(me.id, action, detail).then(() => loadList("activityLog").then(setActivityLog)),
  };
  ...
  const ctx = {
    ...
    fines, rotation, activityLog, persist, notify, activeMembers,
...
function SavingsPage({ ctx }) {
  ...
  const submit = async (e) => {
    ...
    await persist.log("saving_recorded", `${fmtKES(entry.amount)} recorded for ${memberName(entry.memberId)}`);
      ...
      pollRef.current = setInterval(async () => {
        ...
        try {
          ...
          if (statusData.status === "success") {
            ...
            await persist.log("saving_recorded", `${me.name} paid ${fmtKES(mpesaAmount)} via M-PESA`);
            setMpesaAmount("");
    ...
    <div>
      ...
      <div style={{ padding: 16 }}>
        ...
        <Btn variant="ghost" icon={<Plus size={16} />} onClick={() => setShowForm((s) => !s)}>
          {official ? "Record a contribution manually" : "Record a cash contribution"}
        </Btn>
...
function LoansPage({ ctx }) {
  ...
  const submitRequest = async (e) => {
    ...
    await persist.log("loan_requested", `${me.name} requested ${fmtKES(entry.amount)} for "${entry.purpose}"`);
  ...
  const decide = async (loan, decision) => {
    if (decision === "rejected") {
      ...
      await persist.log("loan_rejected", `${me.name} rejected ${fmtKES(loan.amount)} loan for ${memberName(loan.memberId)}`);
    ...
    if (!isLarge(loan)) {
      ...
      await persist.log("loan_approved", `${me.name} approved ${fmtKES(loan.amount)} loan (+${fmtKES(interestAmount)} interest) for ${memberName(loan.memberId)}`);
    ...
    await persist.log("loan_signature", `${me.name} (${roleLabel(me.role)}) signed off on ${fmtKES(loan.amount)} loan for ${memberName(loan.memberId)}`);
  ...
  const recordRepayment = async (loan) => {
    ...
    await persist.log("loan_repayment", `${fmtKES(amt)} repayment recorded for ${memberName(loan.memberId)}'s loan`);
...
function MeetingsPage({ ctx }) {
  ...
  const submit = async (e) => {
    ...
    await persist.log("meeting_logged", `${me.name} logged the ${date} meeting (${attendance.length} present)`);
...
function MoreMenu({ ctx, open, onLogout }) {
  ...
  const items = [
    ...
    official && { key: "activity", label: "Activity log", icon: Clock },
    official && { key: "export", label: "Export data", icon: Download },
  ].filter(Boolean);
...
function MembersPage({ ctx, onBack }) {
  ...
  const approve = async (m) => {
    ...
    await persist.log("member_approved", `${me.name} approved ${m.name}'s registration`);
  ...
  const reject = async (m) => {
    ...
    await persist.log("member_rejected", `${me.name} removed ${m.name}'s pending registration`);
    notify(`${m.name}'s registration removed.`);
  };
  const setRole = async (m, role) => {
    await persist.updateMember(m.id, { role });
    await persist.log("role_changed", `${me.name} set ${m.name}'s role to ${roleLabel(role)}`);
...
function AnnouncementsPage({ ctx, onBack }) {
  const { me, official, announcements, activeMembers, persist, notify } = ctx;
  ...
  const submit = async (e) => {
    ...
    await persist.log("announcement_posted", `${me.name} posted "${entry.title}"`);
...
function FinesPage({ ctx, onBack }) {
  const { me, official, members, fines, persist, notify } = ctx;
  const [showForm, setShowForm] = useState(false);
  ...
  const submit = async (e) => {
    ...
    await persist.log("fine_issued", `${me.name} issued a ${fmtKES(entry.amount)} fine to ${memberName(memberId)} (${entry.reason || "no reason given"})`);
  ...
  const markPaid = async (f) => {
    ...
    await persist.log("fine_paid", `${memberName(f.memberId)}'s ${fmtKES(f.amount)} fine marked paid`);
...
function RotationPage({ ctx, onBack }) {
  const { me, official, members, activeMembers, rotation, persist, notify } = ctx;
  ...
  const resetOrder = async () => {
    ...
    await persist.log("rotation_order_set", `${me.name} (re)set the merry-go-round order (${ids.length} members)`);
  ...
  const advance = async () => {
    ...
    await persist.log("rotation_advanced", `${me.name} advanced the merry-go-round to the next member`);
...
/* -------------------------------- Activity log -------------------------------- */
function ActivityLogPage({ ctx, onBack }) {
  const { members, activityLog } = ctx;
  const memberName = (id) => members.find((m) => m.id === id)?.name || "Unknown";
  const sorted = [...activityLog].sort((a, b) => (a.at < b.at ? 1 : -1));

  return (
    <DetailShell title="Activity log" onBack={onBack}>
      {sorted.length === 0 ? (
        <Card><EmptyState icon={<Clock size={22} color="#C9BE9E" />} text="No activity recorded yet." /></Card>
      ) : (
        <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
          {sorted.map((a) => (
            <Card key={a.id} style={{ padding: "10px 14px" }}>
              <div style={{ fontSize: 13, color: "#3A3524" }}>{a.detail}</div>
              <div style={{ fontSize: 10.5, color: "#A79B78", marginTop: 4 }}>
                {memberName(a.actorId)} · {new Date(a.at).toLocaleString("en-KE")}
              </div>
            </Card>
          ))}
        </div>
...
function ExportPage({ ctx, onBack }) {
  const { members, savings, loans, fines } = ctx;
  const memberName = (id) => members.find((m) => m.id === id)?.name || "Unknown";

  const exporters = [
    {
      label: "Members", count: members.length,
      run: () => downloadCsv("nyish_members.csv", toCsv(members, [
        { label: "Name", value: "name" }, { label: "Phone", value: "phone" },
        { label: "ID Number", value: "idNumber" }, { label: "KRA PIN", value: "kraPin" },
        { label: "Email", value: "email" }, { label: "Role", value: "role" },
        { label: "Status", value: "status" }, { label: "Join date", value: "joinDate" },
      ])),
    },
    {
      label: "Savings", count: savings.length,
      run: () => downloadCsv("nyish_savings.csv", toCsv(savings, [
        { label: "Member", value: (r) => memberName(r.memberId) }, { label: "Amount (KES)", value: "amount" },
        { label: "Date", value: "date" }, { label: "Note", value: "note" },
      ])),
    },
    {
      label: "Loans", count: loans.length,
      run: () => downloadCsv("nyish_loans.csv", toCsv(loans, [
        { label: "Member", value: (r) => memberName(r.memberId) }, { label: "Amount (KES)", value: "amount" },
        { label: "Purpose", value: "purpose" }, { label: "Status", value: "status" },
        { label: "Balance (KES)", value: "balance" }, { label: "Requested", value: "dateRequested" },
        { label: "Approved", value: "dateApproved" },
      ])),
    },
    {
      label: "Fines", count: fines.length,
      run: () => downloadCsv("nyish_fines.csv", toCsv(fines, [
        { label: "Member", value: (r) => memberName(r.memberId) }, { label: "Amount (KES)", value: "amount" },
  ...
  return (
    <DetailShell title="Export data" onBack={onBack}>
      <div style={{ fontSize: 12.5, color: "#5B5138", marginBottom: 14, lineHeight: 1.5 }}>
        Download any table as a CSV file — for the AGM, external accounting, or a backup outside the app.
      </div>
      <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
        {exporters.map((ex) => (
          <Card key={ex.label} style={{ display: "flex", alignItems: "center", justifyContent: "space-between" }}>
            <div>
            ...
            <Btn variant="ghost" icon={<Download size={15} />} onClick={ex.run} disabled={ex.count === 0}>CSV</Btn>
          </Card>
        ))}
      </div>
    </DetailShell>
...
const MorePages = {
  ...
  detail: ({ page, ctx, onBack }) => {
    switch (page) {
      ...
      case "activity": return <ActivityLogPage ctx={ctx} onBack={onBack} />;
