...
import {
  ...
  CheckCircle2, XCircle, Landmark, User, RefreshCw, Eye, EyeOff, ImagePlus
} from "lucide-react";
...
function RegisterScreen({ members, onRegistered, goLogin, notify }) {
  ...
  const submit = async (e) => {
    ...
    if (!form.email.trim()) {
      notify("Email is required — it's how we'll notify you of your registration decision.");
      return;
    }
    ...
    <Card style={{ background: "#FFFDF8" }}>
      ...
      <form onSubmit={submit}>
        ...
        <Field label="Email">
          ...
          <div style={{ fontSize: 11, color: "#A79B78", marginTop: 4 }}>We'll email you when your registration is approved or declined.</div>
        </Field>
...
export default function NyishApp() {
  ...
  const ctx = {
    ...
    fines, rotation, persist, notify, activeMembers,
...
function ExportPage({ ctx, onBack }) {
  ...
  const allExporters = [
    {
      label: "Members", count: members.length, restricted: false,
    ...
    {
      label: "Fines", count: fines.length, restricted: false,
    ...
    {
      label: "Savings", count: savings.length, restricted: true,
    ...
    {
      label: "Loans", count: loans.length, restricted: true,
      run: () => downloadCsv("nyish_loans.csv", toCsv(loans, [
        { label: "Member", value: (r) => memberName(r.memberId) }, { label: "Amount (KES)", value: "amount" },
        { label: "Purpose", value: "purpose" }, { label: "Status", value: "status" },
        { label: "Balance (KES)", value: "balance" }, { label: "Interest (KES)", value: "interestAmount" },
        { label: "Total due (KES)", value: "totalDue" }, { label: "Requested", value: "dateRequested" },
  ...
  return (
    <DetailShell title="Export data" onBack={onBack}>
      <div style={{ fontSize: 12.5, color: "#5B5138", marginBottom: 14, lineHeight: 1.5 }}>
        Download group data as CSV files — for the AGM, accounting, or a backup.
        {!canManageFinance && <span style={{ color: "#8B4A2B" }}> Savings and loans are visible to the Chairperson and Treasurer only.</span>}
      </div>
      <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
        {exporters.map((ex) => (
